...
Use the master pod's headless DNS name (`<name>-0.<name>-headless`):
...
Use the headless service directly (`<name>-headless`):

```bash
valkey-cli -h valkey-headless.<namespace>.svc -p 6380 \
  --tls --cacert /etc/ssl/service-ca/service-ca.crt \
  -a <password>
```
