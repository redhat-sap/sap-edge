...
3. Valkey listens on port **6380** for TLS connections only (`--port 0` disables non-TLS).
4. Cluster bus communication uses plain TCP on port **16380** (OpenShift Service CA certs lack `clientAuth` EKU required for mutual TLS on the bus).
